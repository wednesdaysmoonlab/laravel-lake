# Ubuntu 22.04 Test Server

A bare Ubuntu 22.04 container that simulates a fresh cloud server (DigitalOcean, AWS EC2, GCP Compute Engine, etc.) for testing `lakeup`.
...
```bash
cd environment/os/ubuntu/22.04
...
```bash
docker compose exec lake-ubuntu-22.04 bash
```
...
```bash
docker compose cp ../../../../lakeup lake-ubuntu-22.04:/workspace/lakeup
```
